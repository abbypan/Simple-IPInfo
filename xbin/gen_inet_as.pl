...
#{
##sort and refine twice, to fix ip range cover issue 
...
system(qq[refine_inet.pl $temp.sort.2 $dst.clean.1]);
system(qq[sort -t, -k1,1 -n $dst.clean.1 | uniq > $temp.sort.2]);
system(qq[refine_inet.pl $temp.sort.2 $dst.clean]);
##or
#system(qq[chop_inet.pl $temp $dst.clean]);
#}
